...
flags.DEFINE_integer('num_ps_tasks', 0,
                     ...
                     'If the value is 0, then the parameters are handled locally by the worker.')
...
flags.DEFINE_float('last_layer_gradient_multiplier', 1.0,
                   'The gradient multiplier for last layers, which is used to boost the gradient of last layers '
                   'if the value > 1.')
...
flags.DEFINE_float('drop_path_keep_prob', 1.0,
...
def _tower_loss(iterator, num_of_classes, ignore_label, scope, reuse_variable):
    ...
    tf.summary.scalar('Losses/%s' % regularization_loss.op.name, regularization_loss)
...
def _train_deeplab_model(iterator, num_of_classes, ignore_label):
    ...
    if FLAGS.quantize_delay_step >= 0:
        ...
        tf.contrib.quantize.create_training_graph(quant_delay=FLAGS.quantize_delay_step)
...
def main(unused_argv):
    ...
    with graph.as_default():
        with tf.device(tf.train.replica_device_setter(ps_tasks=FLAGS.num_ps_tasks)):
            ...
            train_tensor, summary_op = _train_deeplab_model(dataset.get_one_shot_iterator(),
            ...
            stop_hook = tf.train.StopAtStepHook(last_step=FLAGS.training_number_of_steps)

            profile_dir = FLAGS.profile_logdir
            if profile_dir is not None:
                tf.gfile.MakeDirs(profile_dir)

            with tf.contrib.tfprof.ProfileContext(enabled=profile_dir is not None, profile_dir=profile_dir):
