...
def _process_batch(sess, original_images, semantic_predictions, image_names,
    ...
    """
    original_images, semantic_predictions, image_names, image_heights, image_widths = \
    ...
    for i in range(num_image):
        ...
        # Save image.
        save_annotation.save_annotation(original_image, save_dir, _IMAGE_FORMAT % (image_id_offset + i),
                                        add_colormap=False)

        # Save prediction.
        save_annotation.save_annotation(crop_semantic_prediction, save_dir, _PREDICTION_FORMAT % (image_id_offset + i),
                                        add_colormap=True, colormap_type=FLAGS.colormap_type)
...
def main(unused_argv):
    ...
    with tf.Graph().as_default():
        ...
        if FLAGS.min_resize_value and FLAGS.max_resize_value:
            ...
            # since we assume the dimensions of original image after tf.squeeze is [height, width, 3].
            assert FLAGS.vis_batch_size == 1

            # Reverse the resizing and padding operations performed in preprocessing.
            # First, we slice the valid regions (i.e., remove padded region) and then we resize the predictions back.
            ...
            resized_image = tf.image.resize_images(tf.expand_dims(predictions, 3), resized_shape,
                                                   method=tf.image.ResizeMethod.NEAREST_NEIGHBOR, align_corners=True)
            predictions = tf.squeeze(resized_image, 3)
        ...
        checkpoints_iterator = tf.contrib.training.checkpoints_iterator(FLAGS.checkpoint_dir,
        ...
        for checkpoint_path in checkpoints_iterator:
            ...
            session_creator = tf.train.ChiefSessionCreator(scaffold=scaffold, master=FLAGS.master,
