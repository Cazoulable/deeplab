...
def _div_maybe_zero(total_loss, num_present):
    ...
    return tf.to_float(num_present > 0) * tf.div(total_loss, tf.maximum(1e-5, num_present))
...
def add_softmax_cross_entropy_loss_for_each_scale(scales_to_logits,
    ...
    for scale, logits in six.iteritems(scales_to_logits):
        ...
        if upsample_logits:
            ...
            logits = tf.image.resize_bilinear(logits, preprocess_utils.resolve_shape(labels, 4)[1:3],
                                              align_corners=True)
            scaled_labels = labels
        else:
            # Label is downsampled to the same size as logits.
            scaled_labels = tf.image.resize_nearest_neighbor(labels, preprocess_utils.resolve_shape(logits, 4)[1:3],
                                                             align_corners=True)
        ...
        if top_k_percent_pixels == 1.0:
          # Compute the loss for all pixels.
            tf.losses.softmax_cross_entropy(one_hot_labels, tf.reshape(logits, shape=[-1, num_classes]),
                                            weights=not_ignore_mask, scope=loss_scope)
        else:
            logits = tf.reshape(logits, shape=[-1, num_classes])
            weights = not_ignore_mask
            with tf.name_scope(loss_scope, 'softmax_hard_example_mining', [logits, one_hot_labels, weights]):
                one_hot_labels = tf.stop_gradient(one_hot_labels, name='labels_stop_gradient')
                pixel_losses = tf.nn.softmax_cross_entropy_with_logits_v2(labels=one_hot_labels,
                                                                          logits=logits,
                                                                          name='pixel_losses')
                ...
                top_k_losses, _ = tf.nn.top_k(weighted_pixel_losses, k=top_k_pixels, sorted=True,
...
def get_model_learning_rate(learning_policy,
    ...
    if slow_start_burnin_type == 'linear':
        ...
        adjustment = (base_learning_rate - slow_start_learning_rate) * tf.to_float(global_step) / slow_start_step
        adjusted_slow_start_learning_rate = slow_start_learning_rate + adjustment
