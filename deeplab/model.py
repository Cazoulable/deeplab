...
def predict_labels_multi_scale(images,
    ...
    for i, image_scale in enumerate(eval_scales):
        ...
        for output in sorted(outputs_to_scales_to_logits):
            ...
            logits = _resize_bilinear(scales_to_logits[MERGED_LOGITS_SCOPE], tf.shape(images)[1:3],
            ...
            if add_flipped_images:
                ...
                logits_reversed = _resize_bilinear(tf.reverse_v2(scales_to_logits_reversed[MERGED_LOGITS_SCOPE], [2]),
                                                   tf.shape(images)[1:3],
...
def predict_labels(images, model_options, image_pyramid=None):
    ...
    for output in sorted(outputs_to_scales_to_logits):
        ...
        if model_options.prediction_with_upsampled_logits:
            logits = _resize_bilinear(logits, tf.shape(images)[1:3], scales_to_logits[MERGED_LOGITS_SCOPE].dtype)
...
def multi_scale_logits(images,
    ...
    for image_scale in image_pyramid:
        ...
        for output in sorted(outputs_to_logits):
            outputs_to_logits[output] = _resize_bilinear(outputs_to_logits[output], [logits_height, logits_width],
...
def extract_features(images,
    ...
    features, end_points = feature_extractor.extract_features(
        ...
        fine_tune_batch_norm=fine_tune_batch_norm,
        nas_stem_output_num_conv_filters=(model_options.nas_stem_output_num_conv_filters),
        nas_training_hyper_parameters=nas_training_hyper_parameters,
