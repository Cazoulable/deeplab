...
flags.DEFINE_list('crop_size', '513,513',
...
def main(unused_argv):
    ...
    with tf.Graph().as_default():
        ...
        model_options = common.ModelOptions(
            ...
            crop_size=[int(i) for i in FLAGS.crop_size],
        ...
        if tuple(FLAGS.inference_scales) == (1.0,):
            ...
            predictions = model.predict_labels_multi_scale(image,
        ...
        semantic_predictions = tf.slice(raw_predictions, [0, 0, 0], [1, resized_image_size[0], resized_image_size[1]])

        # Resize back the prediction to the original image size.
        def _resize_label(label, label_size):
            # Expand dimension of label to [1, height, width, 1] for resize operation.
            label = tf.expand_dims(label, 3)
            resized_label = tf.image.resize_images(label, label_size,
